...
<x-filament-widgets::widget>
    @if (! $shouldShowPaywall)
        {{-- Assinatura ativa: mostra data de expiração --}}
        @if ($subscription?->isActive())
            <x-filament::section>
                <div class="flex items-center gap-4 rounded-2xl border border-success-200 bg-success-50 p-5 dark:border-success-500/30 dark:bg-success-500/10">
                    <x-filament::icon
                    ...
                    <div>
                        <p class="text-sm font-semibold text-success-800 dark:text-success-200">
                            Acesso ao sistema ativo
                        </p>
                        <p class="mt-0.5 text-sm text-success-700 dark:text-success-300">
                            Mensalidade válida até
                            <strong>{{ $subscription->expires_at->timezone('America/Sao_Paulo')->translatedFormat('d \d\e F \d\e Y') }}</strong>
                        </p>
                    </div>
                </div>
            </x-filament::section>
        @endif
    @else
        {{-- Sem assinatura ativa: exibe o payment wall --}}
        <x-filament::section>
            <div class="space-y-6" @if ($hasPaymentRequest) wire:poll.10s="refreshPaymentStatus" @endif>
                ...
                @if ($isApproved)
                    <div class="rounded-2xl border border-success-200 bg-success-50 p-5 text-sm text-success-700 dark:border-success-500/30 dark:bg-success-500/10 dark:text-success-200">
                        Pagamento confirmado. Aguarde alguns segundos ou atualize a página para liberar o acesso.
                    </div>
                @else
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <p class="mt-1 inline-flex rounded-full border px-3 py-1 text-sm font-semibold {{ $isPending ? 'border-amber-300 bg-amber-100 text-amber-900 dark:border-amber-400/70 dark:bg-amber-400/20 dark:text-amber-100' : 'border-slate-300 bg-slate-100 text-slate-800 dark:border-slate-500/70 dark:bg-slate-400/15 dark:text-slate-100' }}">
                                    {{ $paymentRequest?->status ? str($paymentRequest->status)->replace('_', ' ')->title() : 'Aguardando geração' }}
                                </p>
                            </div>
                            ...
                            @if (! $hasPaymentRequest)
                                <div class="rounded-2xl border border-sky-300 bg-sky-50 p-4 text-sm text-sky-950 dark:border-sky-400/70 dark:bg-sky-400/20 dark:text-sky-100">
                                    Clique em <strong>Pagar</strong> para gerar o QR Code Pix. A cobrança fica disponível por 10 minutos.
                                </div>
                            @endif
                            ...
                            @elseif ($paymentRequest?->ticket_url)
                                <div class="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900 dark:border-amber-400/40 dark:bg-amber-400/10 dark:text-amber-100">
                                    A cobrança foi criada, mas o QR Code ainda não foi retornado pelo Mercado Pago neste carregamento.
                                </div>
                            @endif
                            ...
                            @if ($subscription?->expires_at)
                                <div>
                                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Última expiração registrada</p>
                                    <p class="text-sm text-gray-800 dark:text-gray-200">
                            ...
                            <div class="flex flex-wrap gap-3 pt-2">
                                ...
                                >
                                    <span wire:loading.remove wire:target="regeneratePayment">
                                        {{ $hasPaymentRequest ? 'Gerar novo QR Code' : 'Regerar cobrança' }}
                                    </span>
