...
class IntimacaoPreviewService
{
    public function gerarPreview(string $pdfPath, string $token): ?string
    {
        ...
        if (! file_exists($fullPath)) {
            \Illuminate\Support\Facades\Log::warning('IntimacaoPreviewService: arquivo não encontrado', [
                'path' => $fullPath,
            ]);
            return null;
        }
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::warning('IntimacaoPreviewService: erro ao processar imagem', [
                'error' => $e->getMessage(),
                'tmp'   => $jpegTmp,
            ]);
            return null;
